...
// Khởi động giao diện chính sau khi xác nhận mật khẩu
function initMainInterface() {
  ...
  let autoSellRunning = false; // Biến trạng thái cho chức năng bán cá
  ...
  const normalMenuItems = [
    ...
    "Tự động bán toàn bộ cá", // Thêm mục mới
    "Thay đổi tài khoản",
    "Xóa nhật ký",
    "Làm mới",
    "Thoát"
...
${ipLine}`;
  ...
  function updateMenuItems() {
    if (autoTaskRunning || autoFishingRunning || autoDailyRunning || autoSellRunning) {
      mainMenu.setItems([
        ...
        "{gray-fg}Tự động bán toàn bộ cá{/gray-fg}",
  ...
  // Hàm mới: Tự động bán toàn bộ cá
  async function autoSellAllFish() {
    try {
      autoSellRunning = true;
      autoProcessCancelled = false;
      updateMenuItems();
      addLog("{bright-yellow-fg}Bắt đầu tự động bán toàn bộ cá...{/bright-yellow-fg}");

      const agent = getAgent();
      const options = { headers: getRequestHeaders() };
      if (agent) options.agent = agent;

      // Lấy danh sách cá trong kho
      const inventoryResponse = await fetch("https://api.fishingfrenzy.co/v1/inventory/fish", options);
      if (!inventoryResponse.ok) {
        throw new Error(`Lỗi HTTP khi lấy danh sách cá! trạng thái: ${inventoryResponse.status}`);
      }
      const fishList = await inventoryResponse.json();
      
      if (!fishList || fishList.length === 0) {
        addLog("{yellow-fg}Không có cá nào trong kho để bán.{/yellow-fg}");
        autoSellRunning = false;
        updateMenuItems();
        return;
      }

      addLog(`{blue-fg}Đã tìm thấy ${fishList.length} con cá trong kho.{/blue-fg}`);

      // Bán từng con cá
      for (const fish of fishList) {
        if (autoProcessCancelled) {
          addLog("{yellow-fg}Quy trình tự động bán cá đã bị hủy.{/yellow-fg}");
          break;
        }

        const fishId = fish.id; // Giả định mỗi con cá có thuộc tính "id"
        const fishName = fish.fishName || "Không rõ tên"; // Giả định có "fishName"
        const sellPrice = fish.sellPrice || 0; // Giả định có "sellPrice"

        addLog(`{yellow-fg}Đang bán cá: ${fishName} (trị giá ${sellPrice} xu)...{/yellow-fg}`);

        const sellUrl = `https://api.fishingfrenzy.co/v1/inventory/fish/${fishId}/sell`;
        ...
        if (!sellResponse.ok) {
          addLog(`{red-fg}Lỗi khi bán ${fishName}: HTTP ${sellResponse.status}{/red-fg}`);
          continue;
        }

        const sellResult = await sellResponse.json();
        addLog(`{green-fg}Đã bán ${fishName} thành công, nhận được ${sellPrice} xu.{/green-fg}`);
      }

      addLog("{green-fg}Đã hoàn tất quá trình bán toàn bộ cá.{/green-fg}");
    } catch (error) {
      addLog(`{red-fg}Lỗi trong quá trình tự động bán cá: ${error.message}{/red-fg}`);
    } finally {
      autoSellRunning = false;
      updateMenuItems();
      updateUserInfo(); // Cập nhật thông tin người dùng (ví dụ: số vàng tăng sau khi bán)
    }
  }
  ...
  mainMenu.on("select", (item) => {
    ...
    if ((autoTaskRunning || autoFishingRunning || autoDailyRunning || autoSellRunning) && text !== "Dừng tiến trình") {
    ...
    switch (text) {
      ...
      case "Tự động hoàn thành đăng nhập hàng ngày & nhiệm vụ":
        ...
        break;
      case "Tự động bán toàn bộ cá":
        autoSellAllFish(); // Thêm case cho chức năng mới
        break;
...
// Hiển thị hộp nhập mật khẩu khi chương trình khởi động
screen.render();
checkPassword();
